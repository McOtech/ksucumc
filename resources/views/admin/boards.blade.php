    ...
    <div class="card-body">
      <div class="table-responsive">
        <div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4"><div class="row">
                          ...
                          <div class="form-group row">
                            <div class="col-sm-12">
                                <a href="javascript:void(0)">
                                    <img id="newBoardImage" onclick="document.querySelector('#newBoardLogo').click()" class="img-profile rounded-circle d-block m-auto" style="height: 250px; width: 250px" src="{{ asset('storage/default/avatar.png')}}">
                                </a>
          ...
          <tbody>
            <?php $chair = 'No chairperson yet'; $ministry = 0 ?>
            @if (count($cohorts) > 0)
              @foreach ($cohorts as $cohort)
                  @if ($cohort->category == 'board')
                      <tr id="m_{{$cohort->id}}" role="row" class="odd m-category">
                          <td class="sorting_1">
                          <img class="img-profile rounded-circle d-block m-auto" style="height: 50px; width: 50px" src="@if($cohort->image != null){{ asset('' . $cohort->image) }} @else{{ asset('storage/default/avatar.png')}} @endif">
                          </td>
                          <td>{{$cohort->name}}</td>
                          ...
                          <td><?php $sermons = 0; $alerts = 0; $photos = 0; ?>
                              <div class="d-flex">
                                  @foreach ($cohort->permissions as $permission)
                                      @if ($permission != null and $permission->right == 'sermons')
                                        <span>
                                        ...
                                        <?php $sermons = 1?>
                                      @endif
                                      @if ($permission != null and $permission->right == 'alerts')
                                        <span>
                                            <form action="{{route('permission.delete', ['permission' => $permission->id])}}" method="post">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <?php $alerts = 1?>
                                      @endif
                                      @if ($permission != null and $permission->right == 'photos')
                                        <span>
                                            <form action="{{route('permission.delete', ['permission' => $permission->id])}}" method="post">
                                                @csrf
                                                @method('DELETE')
                                  ...
                                  @if ($sermons == 0)
                                    <span>
                                        <form action="{{route('permission.store')}}" method="post">
                                            @csrf
                                            <input type="hidden" name="right" value="sermons">
                                  ...
                                  @if ($alerts == 0)
                                    <span>
                                        <form action="{{route('permission.store')}}" method="post">
                                            @csrf
                                            <input type="hidden" name="right" value="alerts">
                                            <input type="hidden" name="cohort_id" value="{{$cohort->id}}">
                                            <button type="submit" class="btn btn-danger btn-circle btn-sm">
                                            <i class="fas fa-bell"></i>
                                            </button>
                                  ...
                                  @if ($photos == 0)
                                    <span>
                                        <form action="{{route('permission.store')}}" method="post">
                                            @csrf
                                            <input type="hidden" name="right" value="photos">
                                            <input type="hidden" name="cohort_id" value="{{$cohort->id}}">
                                            <button type="submit" class="btn btn-danger btn-circle btn-sm">
                                            <i class="fas fa-images"></i>
                                            </button>
                                      ...
                                      <div class="row">
                                          ...
                                          <form class="user" enctype="multipart/form-data" action="{{ route('cohort.update', ['cohort' => $cohort->id])}}" method="POST">
                                              ...
                                              @method('PATCH')
                                                  ...
                                                  <div class="col-sm-12">
                                                      <a href="javascript:void(0)">
                                                          <img id="updateBoardImage_m_{{$cohort->id}}" onclick="document.querySelector('#updateBoardLogo_m_{{$cohort->id}}').click()" class="img-profile rounded-circle d-block m-auto" style="height: 150px; width: 150px" src="{{ asset('' . $cohort->image) ?? asset('storage/default/avatar.png')}}">
                                                      </a>
            ...
            @if ($ministry == 0)
              <tr class="odd">
                  <td colspan="2">No Board Found</td>
              </tr>
            @endif
        </tbody>

        ...
        <div class="col-xl-12 col-md-6 mb-4">
            <div class="card shadow mb-4 border-bottom-primary">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">{{$cohort->name}} Membership requests</h6>
                </div>
                <div class="card-body">
                      ...
                      <tbody>
                        ...
                        @if ($cohort != null)
                          @foreach ($cohort->membership as $member)
                              @if ($member->right == 'request' && $member->post != 'alumni' && $member->cohort_id == $cohort->id)
                                  <tr role="row" class="odd m-category">
                                    ...
                                    <td>{{$member->user->profile->phone}}</td>
                                      <td>
                                          ...
                                          <span>
                                              <form action="{{route('member.update', ['cohort' => $cohort->id, 'member' => $member->id])}}" method="post">
                                                  @csrf
                                                  @method('PATCH')
                                          ...
                                          <span>
                                              <form action="{{route('member.delete', ['cohort' => $cohort->id, 'member' => $member->id])}}" method="post">
                                                  @csrf
                                                  @method('DELETE')
                                                  <input type="hidden" name="control" value="delete">
                                ...
                                @else
                                {{-- <tr><td>No Member Found {{$notFound}}</td> </tr> --}}
                              @endif
                          @endforeach
                        @endif
                        @if ($notFound == 0)
